...
const handleCheckAI = () => {
  dispatch(checkEligibility({
    hb: parseFloat(form.eligibility.hb) || 0,
    bp: parseFloat(form.eligibility.bp) || 0,
    age: parseFloat(form.eligibility.age) || 0,
    weight: parseFloat(form.eligibility.weight) || 0,
    height: parseFloat(form.eligibility.height) || 0
  }));
};
      ...
      <div className="glass-card p-10 rounded-[3rem] w-full max-w-4xl shadow-2xl border-white/20 animate-in fade-in slide-in-from-bottom-8 duration-700">
        ...
        <div className="flex justify-between mb-12 relative px-10">
          ...
          {[1, 2, 3, 4].map((s) => (
            <div
              key={s}
              className={`w-10 h-10 rounded-full flex items-center justify-center font-bold transition-all ${step >= s ? "bg-primary text-white scale-110 shadow-lg shadow-primary/30" : "bg-slate-100 dark:bg-slate-900 text-slate-400 border border-slate-200 dark:border-slate-800"}`}
            >
              {s}
            </div>
          ))}
          ...
          {step === 1 && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <button
                type="button"
                onClick={() => {
                  setForm({ ...form, donationType: "hospital" });
                  nextStep();
                }}
              ...
              >
                ...
                <p className="text-sm text-slate-500">
                  Donate directly at a registered hospital facility
                </p>
              </button>
              <button
                type="button"
                onClick={() => {
                  setForm({ ...form, donationType: "camp" });
                  nextStep();
                }}
              ...
              >
                ...
                <p className="text-sm text-slate-500">
                  Find a nearby mobile camp or special blood drive
                </p>
              </button>
            </div>
          )}
                  ...
                  {form.donationType === "hospital" ? (
                    ...
                    camps.map((c) => (
                      ...
                      >
                        ...
                        <p className="text-xs text-slate-500">
                          Organized by: {c.hospitalId?.name || "Unknown"}
                        </p>
                    ...
                    <p className="text-center py-10 text-slate-400 font-medium">
                      No active camps found in this district.
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="space-y-8 animate-in slide-in-from-right-8 duration-500">
              ...
              <div className="pt-4">
                <button
                  ...
                  disabled={
                    ...
                    !form.eligibility.hb ||
                    !form.eligibility.bp ||
                    !form.eligibility.weight ||
                    !form.eligibility.height
                  }
                ...
                >
                  {eligibilityLoading
                    ? "AI analyzing..."
                    : "Verify Health with AI 🧠"}
                </button>
              </div>

              {eligibility && (
                <div
                  className={`p-6 rounded-[2.5rem] border-2 animate-in zoom-in-95 duration-500 ${eligibility.status === "Eligible" ? "bg-emerald-50/50 border-emerald-500/20" : "bg-rose-50/50 border-rose-500/20"}`}
                >
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <span
                      className={`px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest ${eligibility.status === "Eligible" ? "bg-emerald-500 text-white" : "bg-rose-500 text-white"}`}
                    >
                      {eligibility.status}
                    </span>
                  </div>
                  ...
                  <div className="space-y-4 max-h-[250px] overflow-y-auto pr-2 custom-scrollbar">
                    {eligibility.explanation?.map((e, idx) => (
                      <div
                        key={idx}
                        className="flex justify-between items-center p-4 bg-white/50 dark:bg-slate-900/50 rounded-2xl shadow-sm border border-white dark:border-slate-800"
                      >
                        ...
                        <div className="text-right ml-4">
                          <p
                            className={`text-[10px] font-black uppercase ${e.impact === "Positive" ? "text-emerald-500" : "text-rose-500"}`}
                          >
                            {e.impact}
                          </p>
                    ...
                    <div className="mt-6 flex items-center justify-center gap-2">
                      <div className="w-full h-1 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden max-w-[100px]">
                        <div
                          ...
                          style={{ width: `${eligibility.confidence * 100}%` }}
                        />
                      </div>
                      <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">
                        Confidence: {(eligibility.confidence * 100).toFixed(0)}%
                      </span>
                    </div>
                  )}
                </div>
              ...
              <div className="flex flex-col gap-4">
                ...
                <button
                  type="button"
                  onClick={prevStep}
                  className="text-slate-400 font-bold text-xs uppercase hover:text-primary transition-colors text-center"
                >
                  ← Go Back
                </button>
              </div>
            ...
            <div className="space-y-8 animate-in slide-in-from-right-8 duration-500">
              ...
              <div className="pt-6 border-t border-slate-200 dark:border-slate-800">
                ...
                <button
                  type="button"
                  onClick={prevStep}
                  className="w-full text-slate-400 font-bold text-xs uppercase mt-6 hover:text-primary transition-colors"
                >
                  ← Edit Details
                </button>
              </div>
