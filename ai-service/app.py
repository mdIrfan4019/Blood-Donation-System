...
#         hb = get_value(data["hb"])
#         age = get_value(data["age"])
#         bp = get_value(data["bp"])
#         weight = get_value(data["weight"])
#         height = get_value(data["height"])
...
def predict_eligibility():
    ...
    try:
        ...
        # FIX: Explicitly extract the positive class [1] values for a Random Forest
        shap_explanation = shap_explainer(features_df)
        shap_values_class_1 = shap_explanation.values[0, :, 1]

        shap_map = {
            feature: round(float(value), 4)
            for feature, value in zip(features_df.columns, shap_values_class_1)
        }

        # Attach SHAP to explanation items
        for e in explanation:
            if "Hemoglobin" in e["factor"]:
                e["shap"] = shap_map.get("hb")
            elif "Age" in e["factor"]:
